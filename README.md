...
비밀번호 찾기를 사용하려면 `FRONTEND_URL`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_FROM_EMAIL`을 설정해야 합니다. SMTP 인증이 필요한 경우 `SMTP_USERNAME`, `SMTP_PASSWORD`도 설정합니다. 설정하지 않은 서버는 성공을 가장하지 않고 사용자에게 메일 기능이 준비되지 않았다고 안내합니다.
